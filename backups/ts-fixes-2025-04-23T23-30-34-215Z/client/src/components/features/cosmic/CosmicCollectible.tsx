...
/**
 * Original CosmicCollectible component merged from: client/src/components/common/cosmic-collectible.tsx
 * Merge date: 2025-04-05
 */
function CosmicCollectibleOriginal({
  id,
  name,
  description,
  image,
...
// Collection grid component (original version) - kept for reference
function CosmicCollectiblesGridOriginal({
  collectibles,
  onClaim,
}: {
  collectibles: CollectibleProps[]
  ...
  return (
    <div className="grid grid-cols-2 md:grid-cols-3 lg:grid-cols-4 gap-4 md:gap-6">
      {collectibles.map((collectible) => (
        <CosmicCollectibleOriginal key={collectible.id} {...collectible} onClaim={onClaim} />
      ))}
    </div>
  )
}

// Wallet connection component (original version) - kept for reference
function CosmicWalletConnectOriginal({
  isConnected = false,
  address = "",
  onConnect,
  onDisconnect,
