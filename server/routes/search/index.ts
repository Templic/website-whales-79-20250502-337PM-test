 ...
 * @apiParam {String} [type] Content type: 'all', 'music', 'products', 'posts', 'users'
...
router.get('/', async (req: Request, res: Response) => {
  try {
    ...
    const [music, products, posts, users] = await Promise.all(promises);
    
    // Populate results based on the type
    if (type === 'all' || type === 'music') results.music = music || [];
    if (type === 'all' || type === 'products') results.products = products || [];
    if (type === 'all' || type === 'posts') results.posts = posts || [];
