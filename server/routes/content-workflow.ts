...
// Get content workflow metrics for dashboard (admin only)
router.get('/metrics', requireAuth, requireRole(['admin', 'super_admin']), async (req, res) => {
  try {
    // Get counts of content in different states
    const draftsCount = await db.select({ count: db.fn.count() })
      .from(contentItems)
      .where(eq(contentItems.status, 'draft'));
    
    const inReviewCount = await db.select({ count: db.fn.count() })
      .from(contentItems)
      .where(eq(contentItems.status, 'review'));
    
    const changesRequestedCount = await db.select({ count: db.fn.count() })
      .from(contentItems)
      .where(eq(contentItems.status, 'changes_requested'));
    
    const approvedCount = await db.select({ count: db.fn.count() })
      .from(contentItems)
      .where(eq(contentItems.status, 'approved'));
    
    const publishedCount = await db.select({ count: db.fn.count() })
      .from(contentItems)
      .where(eq(contentItems.status, 'published'));
    
    const archivedCount = await db.select({ count: db.fn.count() })
      .from(contentItems)
      .where(eq(contentItems.status, 'archived'));
    ...
    const recentApprovals = await db.select()
      .from(contentWorkflowHistory)
      .where(and(
        eq(contentWorkflowHistory.action, 'approved'),
        gte(contentWorkflowHistory.createdAt, last30Days)
      ));
    ...
    for (const approval of recentApprovals) {
      const submission = await db.select()
        .from(contentWorkflowHistory)
        .where(and(
          eq(contentWorkflowHistory.contentId, approval.contentId),
          eq(contentWorkflowHistory.action, 'submitted')
        ))
        .orderBy(desc(contentWorkflowHistory.createdAt))
        .limit(1);
      
      if (submission.length > 0) {
        const submissionDate = new Date(submission[0].createdAt);
        const approvalDate = new Date(approval.createdAt);
        const timeDiff = (approvalDate.getTime() - submissionDate.getTime()) / (1000 * 60 * 60 * 24); // in days
    ...
    const recentActivity = await Promise.all(last7Days.map(async (dateString) => {
      const startDate = startOfDay(parseISO(dateString));
      const endDate = endOfDay(parseISO(dateString));
      
      const newContentCount = await db.select({ count: db.fn.count() })
        .from(contentItems)
        .where(and(
          gte(contentItems.createdAt, startDate),
          lte(contentItems.createdAt, endDate)
        ));
      
      const publishedCount = await db.select({ count: db.fn.count() })
        .from(contentWorkflowHistory)
        .where(and(
          eq(contentWorkflowHistory.action, 'published'),
          gte(contentWorkflowHistory.createdAt, startDate),
          lte(contentWorkflowHistory.createdAt, endDate)
        ));
      
      const reviewsCount = await db.select({ count: db.fn.count() })
        .from(contentWorkflowHistory)
        .where(and(
          eq(contentWorkflowHistory.action, 'submitted'),
          gte(contentWorkflowHistory.createdAt, startDate),
      ...
      return {
        date: dateString,
        newContent: parseInt(newContentCount[0].count.toString(), 10),
        published: parseInt(publishedCount[0].count.toString(), 10),
        reviews: parseInt(reviewsCount[0].count.toString(), 10)
      };
    }));
    
    // Prepare status distribution data
    const statusDistribution = [
      { name: 'Draft', value: parseInt(draftsCount[0].count.toString(), 10) },
      { name: 'In Review', value: parseInt(inReviewCount[0].count.toString(), 10) },
      { name: 'Changes Requested', value: parseInt(changesRequestedCount[0].count.toString(), 10) },
      { name: 'Approved', value: parseInt(approvedCount[0].count.toString(), 10) },
      { name: 'Published', value: parseInt(publishedCount[0].count.toString(), 10) },
      { name: 'Archived', value: parseInt(archivedCount[0].count.toString(), 10) }
    ];
    
    // Compile metrics
    const metrics = {
      drafts: parseInt(draftsCount[0].count.toString(), 10),
      inReview: parseInt(inReviewCount[0].count.toString(), 10),
      changesRequested: parseInt(changesRequestedCount[0].count.toString(), 10),
      approved: parseInt(approvedCount[0].count.toString(), 10),
      published: parseInt(publishedCount[0].count.toString(), 10),
      archived: parseInt(archivedCount[0].count.toString(), 10),
      totalContent: parseInt(totalCount[0].count.toString(), 10),
      averageApprovalTime: parseFloat(averageApprovalTime.toFixed(2)),
      recentActivity,
      statusDistribution
    };
    ...
    logger.error('Error getting content workflow metrics:', error);
    res.status(500).json({ error: 'Failed to get content workflow metrics' });
  }
});
