...
export interface IStorage: {
  ...
  // Subscriber methods: createSubscriber(subscriber: InsertSubscriber): Promise<Subscriber>;
  getAllSubscribers(): Promise<Subscriber[]>;
  findSubscriberByEmail(email: string): Promise<Subscriber | undefined>;

  // Newsletter methods: createNewsletter(newsletter: InsertNewsletter): Promise<Newsletter>;
  ...
  // Post methods: createPost(post: InsertPost): Promise<Post>;
  getPosts(): Promise<Post[]>;
  getAllPosts(): Promise<Post[]>; // Method for getting all posts including unpublished ones: getPostById(id: number): Promise<Post | null>;
  updatePost(id: number, post: Partial<InsertPost>): Promise<Post>;

  // Category methods: createCategory(category: InsertCategory): Promise<Category>;
  getCategories(): Promise<Category[]>;

  // Comment methods: createComment(comment: InsertComment): Promise<Comment>;
  getCommentsByPostId(postId: number): Promise<Comment[]>;
  approveComment(id: number): Promise<Comment>;
  rejectComment(id: number): Promise<Comment>;

  // Password recovery methods: createPasswordResetToken(userId: number): Promise<string>;
  validatePasswordResetToken(token: string): Promise<User | undefined>;
  updateUserPassword(userId: number, newPassword: string): Promise<User>;

  // Admin methods for content moderation: approvePost(id: number): Promise<Post>;
  getUnapprovedPosts(): Promise<Post[]>;
  getUnapprovedComments(): Promise<Comment[]>;

  // Music methods: getTracks(): Promise<Track[]>;
  getAllTracks(): Promise<Track[]>; // Get all tracks including non-published ones: getAlbums(): Promise<Album[]>;
  ...
  // Product methods: getAllProducts(): Promise<Product[]>;

  // Session management methods: cleanupExpiredSessions(): Promise<void>;
  getSessionAnalytics(userId: number): Promise<any>;
  updateSessionActivity(sessionId: string, data): Promise<void>;

  // Advanced admin methods: updateUserRole(userId: number, role: 'user' | 'admin' | 'super_admin'): Promise<User>;
  ...
  // Content management methods: getAllContentItems(): Promise<ContentItem[]>;
  ...
  // Content versioning methods: getContentHistory(contentId: number): Promise<ContentHistory[]>;
  ...
  // Content usage tracking methods: recordContentUsage(contentId: number, location: string, path: string): Promise<ContentUsage>;
  incrementContentViews(contentId: number): Promise<void>;
  getContentUsageReport(contentId?: number): Promise<any[]>;
  
  // New content workflow methods: getContentWorkflowHistory(contentId: number): Promise<ContentWorkflowHistory[]>;
  updateContentStatus(
    contentId: number, 
    status: string, 
    userId: number, 
