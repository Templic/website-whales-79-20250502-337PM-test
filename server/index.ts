...
if (isReplitEnv && process.env.NODE_ENV !== 'production') {
  log('Replit development environment detected - using standalone tools for API validation', 'server');
}
...
// Always disable validation test routes to ensure the main application loads correctly
const enableValidationTestRoutes = false; // Explicitly disable regardless of environment
log('Direct validation test routes explicitly disabled', 'server');
