 ...
 * A player that can be controlled with voice commands
 */
"use client"
    ...
    <div className="rounded-xl bg-black/30 backdrop-blur-sm border border-purple-500/20 overflow-hidden">
      ...
      <div className="p-6 grid md:grid-cols-2 gap-6">
        ...
        <div className="space-y-6">
          ...
          <div className="rounded-lg bg-black/40 p-4 space-y-4">
              ...
              <div className="space-y-3">
                        ...
                        <div className="flex items-center gap-2">
                          <div className="flex space-x-1">
                            {Array.from({ length: 5 }).map((_, i) => (
                              <div
                                key={i}
                                className="w-1 h-16 bg-purple-500/50 animate-pulse"
                                style={{
                          ...
                          <span className="text-sm text-white/80">{transcript || "Say a command..."}</span>
                        </div>
                      </div>
                    </>
                  ) : (
                ...
                {lastCommand && (
                  <div className="rounded-md bg-purple-500/10 border border-purple-500/20 p-2">
                    <p className="text-sm">
                      <span className="font-medium text-purple-400">Last command:</span>{" "}
            ...
            {!voiceEnabled && (
              <div className="p-3 rounded-md bg-white/5 text-center">
                <p className="text-sm text-white/60">Toggle switch to enable voice control</p>
              </div>
            )}
          </div>
          ...
          <div className="rounded-lg bg-black/40 p-4 space-y-3">
            <h3 className="font-medium text-white">Track List</h3>
            <div className="max-h-[200px] overflow-y-auto space-y-2 pr-2 scrollbar-thin scrollbar-thumb-white/20 scrollbar-track-transparent">
              {tracks.map((track, index) => (
                <div
                  key={track.id}
                  className={cn(
                    "flex items-center gap-3 p-2 rounded-md hover:bg-white/5 cursor-pointer transition-colors",
                    currentTrackIndex === index ? "bg-purple-500/20 border border-purple-500/30" : "border border-transparent",
                  )}
                  onClick={() => {
                    setCurrentTrackIndex(index)
                    setIsPlaying(true)
                ...
                >
                  <div className="w-8 h-8 rounded overflow-hidden bg-black/40 flex-shrink-0">
                    <img src={track.coverArt || "/placeholder.svg"} alt={track.title} className="w-full h-full object-cover" />
                  </div>
                  <div className="flex-grow min-w-0">
                    <h4 className="font-medium text-white truncate">{track.title}</h4>
                    <p className="text-xs text-white/60 truncate">{track.artist}</p>
                  </div>
                  <span className="text-xs text-white/40 flex-shrink-0">{track.duration}</span>
                </div>
              ))}
            </div>
          </div>
      ...
      {showCommands && (
        <div className="absolute inset-0 bg-black/80 backdrop-blur-sm flex items-center justify-center z-50 p-4">
          <div className="bg-gray-900/90 border border-purple-500/20 rounded-xl max-w-md w-full max-h-[80vh] overflow-auto">
            <div className="p-4 border-b border-white/10 flex items-center justify-between">
            ...
            <div className="p-4">
              <p className="text-sm text-white/60 mb-4">
                You can control the player using the following voice commands:
              </p>
              <div className="space-y-2">
              ...
              <div className="mt-4 p-3 rounded-md bg-purple-500/10 border border-purple-500/20">
                <div className="flex items-center gap-2">
                  ...
                  <p className="text-xs text-white/80">
                    Voice control requires microphone permissions and works best in a quiet environment. Make sure to
