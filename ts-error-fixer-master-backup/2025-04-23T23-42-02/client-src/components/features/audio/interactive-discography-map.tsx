...
export function InteractiveDiscographyMap() {
  ...
  const handleMouseMove = (e: React.MouseEvent) => {
    ...
    if (mapContainerRef.current) {
      ...
      setCursorPosition({ x: parseFloat(x), y: parseFloat(y) });
    }
  ...
  return (
    <div className="rounded-xl bg-black/30 backdrop-blur-sm border border-purple-500/20 overflow-hidden">
      ...
      >
        ...
        {/* Coordinates display */}
        {cursorPosition && (
          <div className="absolute top-4 left-4 bg-black/60 text-white/90 text-xs px-2 py-1 rounded backdrop-blur-sm z-10">
            Coordinates: {cursorPosition.x}%, {cursorPosition.y}%
          </div>
        )}
