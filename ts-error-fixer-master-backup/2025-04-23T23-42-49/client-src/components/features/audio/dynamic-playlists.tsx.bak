...
export function DynamicPlaylists({ playlists, onPlay, className }: DynamicPlaylistsProps) {
  ...
  const categories = ["all", ...new Set(playlists.map((playlist) => playlist.category))]
...
export function RecommendedPlaylists({
  playlists,
  onSelect,
}: { playlists: Playlist[]; onSelect?: (playlist: Playlist) => void }) {
  return (
    <div>
...
/**
 * Original DynamicPlaylists component merged from: client/src/components/common/dynamic-playlists.tsx
 * Merge date: 2025-04-05
 */
function DynamicPlaylistsOriginal({ playlists, onPlay, className }: DynamicPlaylistsProps) {
  const [activePlaylist, setActivePlaylist] = useState<Playlist | null>(null)
  const [filter, setFilter] = useState<string>("all")
  const [isExpanded, setIsExpanded] = useState(false)
...
/**
 * Original RecommendedPlaylists component merged from: client/src/components/common/dynamic-playlists.tsx
 * Merge date: 2025-04-05
 */
function RecommendedPlaylistsOriginal({
  playlists,
  onSelect,
}: { playlists: Playlist[]; onSelect?: (playlist: Playlist) => void }) {
  return (
...
/**
 * Original DynamicPlaylists component merged from: client/src/components/music/DynamicPlaylists.tsx
 * Merge date: 2025-04-05
 */
function DynamicPlaylistsSecondOriginal({ playlists, onPlay, className }: DynamicPlaylistsProps) {
  const [activePlaylist, setActivePlaylist] = useState<Playlist | null>(null);
  const [filter, setFilter] = useState<string>("all");
  const [isExpanded, setIsExpanded] = useState(false);
...
/**
 * Second original RecommendedPlaylists component from another source
 * Merge date: 2025-04-05
 */
function RecommendedPlaylistsSecondOriginal({
  playlists,
  onSelect,
}: {
  playlists: Playlist[];
